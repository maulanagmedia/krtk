...
public class ImageUtils {
    ...
    public void LoadRealImage(Context context, int uri, final ImageView image){
        ...
        Picasso.with(context).load(uri).networkPolicy(NetworkPolicy.NO_CACHE).memoryPolicy(MemoryPolicy.NO_CACHE).error(context.getResources().getDrawable(R.mipmap.ic_thumbnail)).placeholder(context.getResources().getDrawable(R.mipmap.ic_thumbnail)).into(image);
    }
    ...
    public void LoadSquareImageHeaderSlider(Context context, String uri, final ImageView image, int size){
        ...
        Picasso.with(context).load(Uri.parse(uri)).networkPolicy(NetworkPolicy.NO_CACHE).resize(size, size).memoryPolicy(MemoryPolicy.NO_CACHE).error(context.getResources().getDrawable(R.mipmap.ic_thumbnail)).placeholder(context.getResources().getDrawable(R.mipmap.ic_thumbnail)).into(image);
    }

    public void LoadCustomSizedImage(Context context, int uri, final ImageView image, int width, int height){

        Picasso.with(context).load(uri).networkPolicy(NetworkPolicy.NO_CACHE).memoryPolicy(MemoryPolicy.NO_CACHE).resize(width, height).centerCrop().error(context.getResources().getDrawable(R.mipmap.ic_thumbnail)).placeholder(context.getResources().getDrawable(R.mipmap.ic_thumbnail)).into(image);
    }
